...
local function status(y, msg)
  if gpu then
    gpu.set((w/2)-(#msg/2), y, msg)
  end
...
gpu.fill(1, (h/2)-1, w, (h/2)+1, " ")
status((h/2)-1, "Starting package library")

-- Load file system related libraries we need to load other stuff moree
-- comfortably. This is basically wrapper stuff for the file streams
-- provided by the filesystem components.
local package = dofile("/d0A/lib/package.lua")
...
do
  ...
  package.loaded.buffer = dofile("/d0A/lib/buffer.lua")
  package.loaded.filesystem = dofile("/d0A/lib/filesystem.lua")

  -- Inject the io modules
  _G.io = dofile("/d0A/lib/io.lua")
end
gpu.fill(1, (h/2)-1, w, (h/2)+1, " ")
status((h/2)-1, "Initializing file system")
...
gpu.fill(1, (h/2)-1, w, (h/2)+1, " ")
status((h/2)-1, "Running boot scripts...")
...
gpu.fill(1, (h/2)-1, w, (h/2)+1, " ")
status((h/2)-1, "Initializing components...")
...
gpu.fill(1, (h/2)-1, w, (h/2)+1, " ")
status((h/2)-1, "Initializing system...")

computer.pushSignal("init") -- so libs know components are initialized.
require("centertext")(h/2, "Welcome to TherOS! Main environment will load shortly...")
